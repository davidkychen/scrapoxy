...
module.exports = class ProviderGCP {
    ...
    get models() {
            ...
            return new Promise((resolve, reject) => {
                self._zone.getVMs(
                    ...
                    (err, vms, apiResponse) => {
                        ...
                        winston.debug(
                            "[ProviderGCP] describeInstances get VMs:",
                            _.map(vms, "name")
                        );
                        resolve(vms);
                    }
        ...
        function excludeOutscope(instancesDesc) {
            return _.filter(
                ...
                (instanceDesc) =>
                    instanceDesc.tag &&
                    instanceDesc.tag.indexOf(self._config.tag) >= 0
            );
        }

        function convertToModel(instancesDesc) {
            winston.debug(
                "[ProviderGCP] convertToModel:",
                instancesDesc.length
            );
            return _.map(
                instancesDesc,
                (instanceDesc) =>
            ...
            function convertStatus(status) {
                switch (status.toUpperCase()) {
                    ...
                    default: {
                        winston.error(
                            "[ProviderGCP] Error: Found unknown status:",
                            status
                        );
    ...
    createInstances(count) {
        ...
        async function _createVM(zone, vmName, config) {
            ...
            winston.debug(
                "[ProviderGCP] createInstances: VM created successfully"
            );
            return vm.id;
        }

        function createInstances(instanceConfig, cnt) {
            return self.models.then(
                (instances) =>
                    new Promise((resolve, reject) => {
                        ...
                        _.times(cnt, (n) => {
                            vmPromises.push(
                                _createVM(
                                    self._zone,
                                    `scraproxy-instance-${instanceCount + n}`,
                                    instanceConfig
                                )
                            );
                        });
